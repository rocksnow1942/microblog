...
def explore():
    page = request.args.get('page',1,type=int)
    posts = Post.query.filter(Post.user_id!=current_user.id).order_by(Post.timestamp.desc()).paginate(
            page,current_app.config['POSTS_PER_PAGE'], False)
...
def unfollow(username):
    ...
    db.session.commit()
    flash(_('Your have unfollowed %(username)s.',username=username))
    return redirect(url_for('main.user',username=username))
...
def notifications():
    ...
    return jsonify([{'name':n.name, 'data':n.get_data(),'timestamp':float(n.timestamp)} for n in notifications])
